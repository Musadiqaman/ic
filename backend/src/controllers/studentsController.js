...
function recomputeAttendance(student) {
  ...
  student.attendancePercent = total ? Math.round((present / total) * 100) : 0;
}
...
export const list = asyncHandler(async (req, res) => {
  // Lazily make sure this month's challans exist every time the list is
  // loaded, instead of relying purely on a cron job that may not be running.
  try {
    await generateMonthlyChallans();
  } catch (e) {
    console.error("Auto challan generation failed:", e.message);
  }
...
export const markAttendance = asyncHandler(async (req, res) => {
  ...
  student.attendanceHistory.push({
    ...
    status,
    note: note || "",
    type: type === "face-scan" ? "face-scan" : "manual",
  });
...
export async function runAutoAttendance(forDateStr) {
  ...
  const weekday = new Date(`${dateStr}T00:00:00`).getDay(); // 0 = Sunday ... 6 = Saturday
  ...
  return { date: dateStr, leaveCount, absentCount, studentsChecked: students.length };
}
...
export const runAutoAttendanceNow = asyncHandler(async (req, res) => {
  ...
  res.json({
    message: `Auto-attendance applied for ${result.date}: ${result.leaveCount} marked leave, ${result.absentCount} marked absent.`,
    ...result,
  });
});
