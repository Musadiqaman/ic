...
import { ArrowUpRight, ArrowDownRight, Fingerprint, Loader2, CalendarRange } from "lucide-react";
...
function useCountUp(target, duration = 1200) {
  ...
  useEffect(() => {
    let raf;
    startRef.current = null; // reset so re-triggering (new `target`) animates from 0 again
    const step = (t) => {
...
export default function Dashboard() {
  ...
  const cashInHand = useCountUp(summary?.cashInHand || 0);
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 transition-opacity duration-200" style={{ opacity: loading ? 0.55 : 1 }}>
        <Card
